...
- 总 loss = masked L2 reconstruction。
- 每个 sequence 都重新开始 state=None。
- 输入和输出都只使用 INPUT_VIDEO_PATH 指定的单个视频。
...
# 唯一输入视频：既用于训练，也用于每个 epoch 后的整段去噪输出。
INPUT_VIDEO_PATH = Path(
    r"C:\Users\Novovorontsovka\Downloads\video_masqued\output\05_trimmed_videos\R_260310_AUZ0752_4_p_circle_masked_circle_masked_from_first_peak.avi"
)

# 与该视频对应的 smooth brightness table 所在文件夹。
BRIGHTNESS_DIR = Path(
    r"C:\Users\Novovorontsovka\Downloads\video_masqued\output\04_brightness_tables"
)

# 每个 epoch 以及最佳模型的去噪视频输出位置。
DENOISED_OUTPUT_DIR = Path(
    r"C:\Users\Novovorontsovka\Downloads\video_masqued\output\05_denoised_video__l2"
)



# 当前唯一处理的视频编号。
OUTPUT_VIDEO_NUMBER = 4
...
# ============================================================
# 3.2 读取一个视频对应的平滑亮度和 phase table
# ============================================================
...
class SingleVideoDataset(Dataset):
    ...
    def __init__(
        ...
        # 为了不改变后续训练、遮挡和验证代码的接口，
        # 单个视频仍统一使用 video_number=0 作为字典键。
        self.video_number = 0
        ...
        if (
            ...
            raise RuntimeError(
                "输入视频尺寸不符合要求：\n"
                f"actual={frame_width}x{frame_height}\n"
        ...
        if total_frames < self.sequence_length:
            ...
            raise RuntimeError(
                "输入视频帧数不足，无法组成一条完整 sequence：\n"
                f"video frames = {total_frames}\n"
                f"required frames = {self.sequence_length}\n"
                f"{self.video_path}"
            )
        ...
        if len(frame_indices) != total_frames:
            ...
            raise RuntimeError(
                "输入视频帧数和 brightness table 长度不一样：\n"
                f"video frames = {total_frames}\n"
...
# 训练和整段推理均只使用 INPUT_VIDEO_PATH 指定的单个视频。
test_video_paths = [INPUT_VIDEO_PATH]

print(
    "Single training/output video:",
    INPUT_VIDEO_PATH
)
...
# ============================================================
# 9. Loss：只使用 masked L2 reconstruction
#
# 不使用 gradient、Hessian、continuity、VFC 或人工 vessel mask。
# loss 只在当前帧被替换的 block 内计算。
# ============================================================
...
def masked_l2_loss(
    ...
    l2_loss_per_sample = masked_mean_per_sample(
        torch.square(
            prediction - target
        ),
        block_mask
    ...
    batch_total_loss = l2_loss_per_sample.sum()
...
def train_one_sequence(
    ...
    current_loss, l2_loss_per_sample = masked_l2_loss(
...
def print_one_sample_loss(
    ...
    print(
        ...
        f"L2={float(l2_loss):.8f}",
        flush=True
    )


# ============================================================
# 15.8 从当前单个视频的训练 split 中抽取 sequence
# ============================================================
...
def select_train_samples_for_epoch(
    ...
    # 当前只有一个视频；先保留 1 条属于 train split 的 sequence。
    for video_number in sorted(
            dataset.sample_indices_by_video.keys()
    ):
    ...
    if len(mandatory_indices) > train_samples_per_epoch:
        ...
        raise RuntimeError(
            "保留的必选 sequence 数量大于每个 epoch 的 sample 数，"
            "无法保证每个视频至少出现一次。\n"
            f"mandatory train sequences = {len(mandatory_indices)}\n"
...
def denoise_selected_test_videos(
    ...
    for source_video_path in test_video_paths:
        ...
        # 当前版本只有一个输入/输出视频，不再进行测试编号筛选。
        output_path = output_directory / (
            f"{OUTPUT_VIDEO_NUMBER}_denoised.avi"
        )
...
MODEL_SAVE_PATH = Path(
    r"C:\Users\Novovorontsovka\Downloads\model\best_video9_block32_convlstm.pth"
)

# 每个 epoch 结束后保存当时模型的 state_dict。
# 之后可以直接拿任意一个 epoch_xxx.pth 做推理。
EPOCH_MODEL_DIR = Path(
    r"C:\Users\Novovorontsovka\Downloads\model\epoch_models_video9_block32_convlstm"
)
...
model = train_model(
    ...
    r=R,
    test_video_paths=test_video_paths,
    test_output_root=DENOISED_OUTPUT_DIR
)   
